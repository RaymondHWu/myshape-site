...
export default function GenesisCohortBadge() {
  ...
  if (error || count === null) return null; // silent failure — non-critical
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {/* Count */}
      <span className="text-[#90c8ff]/50 text-[10px] tracking-[0.12em] font-mono whitespace-nowrap">
        <span className="text-[#90c8ff]/80">{count}</span>
        <span className="text-[#90c8ff]/25"> / {GENESIS_TARGET}</span>
        <span className="text-[#90c8ff]/20 ml-1">GENESIS</span>
      </span>
    </div>
  );
}
