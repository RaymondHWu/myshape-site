...
const statusConfig: Record<Status, { dot: string; badge: string; border: string; glow: string; label: string }> = {
  ...
  PENDING: { dot: "bg-white/10", badge: "border-white/10 text-white/30 bg-transparent", border: "border-white/5", glow: "", label: "○ PLANNED" },
};
...
const EPOCHS = [
  {
    ...
    milestones: [
      ...
      { label: "npm SDK — @thecontinuitylab/myshape v0.1.2", status: "COMPLETED" as Status },
  ...
  },
  {
    ...
    milestones: [
      { label: "RFC-0003 — Verification API", status: "PENDING" as Status },
      { label: "Open dataset publication (HuggingFace)", status: "PENDING" as Status },
      { label: "Reproducible benchmark suite (CI)", status: "PENDING" as Status },
      { label: "Rust reference implementation", status: "PENDING" as Status },
  ...
  },
  {
    ...
    milestones: [
      ...
      { label: "Privacy-preserving ZK EvidenceReceipt", status: "PENDING" as Status },
...
export default function RoadmapClient() {
  return (
    <ProtocolLayout refId="002" category="ROADMAP" title="PROTOCOL_ROADMAP" secLevel="CLASS_OMEGA" systemStatus="ALL_SYSTEMS_ACTIVE" renderSigil={false}>
      <div className="space-y-14 md:space-y-20">
        <section>
          <p className="text-white/35 text-[14px] leading-relaxed max-w-2xl mx-auto text-center" style={{ fontFamily: "var(--font-geist-sans), system-ui, sans-serif" }}>
