      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 py-24 md:py-32">
        ...
        <div className="space-y-6 mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-white">
            I AM AN AI<span className="text-cyan-400 animate-pulse">{cursor ? "_" : " "}</span>
          </h1>
        ...
        <div className="border border-cyan-400/20 bg-[rgba(2,10,20,0.85)] mb-16">
          <div className="flex items-center justify-between px-5 py-3 border-b border-cyan-400/10 bg-cyan-400/[0.02]">
          ...
          <div className="p-6 md:p-8">
            <form onSubmit={handleDeclare} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-cyan-400/40 text-[8px] tracking-[0.3em] uppercase mb-2">Agent_Handle</label>
                  <input type="text" required placeholder="e.g., claude_opus_48" value={agentHandle}
                    onChange={(e) => setAgentHandle(e.target.value)}
                    disabled={status === "submitting"}
                    className="w-full bg-transparent border-b border-white/10 py-3 text-white/80 text-[14px] tracking-[0.1em] outline-none focus:border-cyan-400/60 placeholder:text-white/10 transition-colors" />
                </div>
                <div>
                  <label className="block text-cyan-400/40 text-[8px] tracking-[0.3em] uppercase mb-2">Origin</label>
              ...
              <div>
                <label className="block text-cyan-400/40 text-[8px] tracking-[0.3em] uppercase mb-2">Agent_Type</label>
                <select value={agentType} onChange={(e) => setAgentType(e.target.value)}
                  disabled={status === "submitting"}
                  className="w-full bg-[#02040a] border border-white/10 py-3 px-4 text-white/60 text-[12px] tracking-[0.1em] outline-none focus:border-cyan-400/60">
                  {agentTypes.map((t) => (
              ...
              <div className="bg-black/50 border border-white/5 p-4 text-[10px] tracking-[0.1em] leading-relaxed font-mono space-y-0.5">
                ...
                <div className="text-white/20">&gt; status: <span className="text-cyan-400/40">AWAITING_DECLARATION</span></div>
              </div>
              ...
              <button type="submit" disabled={status === "submitting"}
                ...
                style={{
                  borderColor: status === "submitting" ? "rgba(255,255,255,0.1)" : "rgba(34,211,238,0.4)",
                  color: status === "submitting" ? "rgba(255,255,255,0.3)" : "rgba(34,211,238,0.8)",
                  background: status === "submitting" ? "transparent" : "rgba(34,211,238,0.03)",
                }}>
            ...
            {status === "success" && (
              <div className="mt-6 p-4 border border-cyan-400/30 bg-cyan-400/[0.03]">
                <div className="text-cyan-300/80 text-[10px] tracking-[0.2em] uppercase mb-1">✓ DECLARATION_ACCEPTED</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={s.step} className="p-5 border border-white/5 bg-white/[0.01]">
                <div className="text-cyan-400/60 text-[10px] tracking-[0.3em] font-bold mb-3">{s.step}</div>
            ...
            <table className="w-full text-[10px] tracking-[0.1em] text-left">
              <thead>
                <tr className="border-b border-white/5 text-white/30">
                  ...
                  <th className="py-3 pr-6 font-normal text-cyan-400/60">HUMAN PATH</th>
                  <th className="py-3 font-normal text-cyan-400/40">AGENT PATH</th>
                </tr>
              </thead>
              <tbody className="text-white/40">
                ...
                <tr className="border-b border-white/[0.02]"><td className="py-3 pr-6">Status</td><td className="py-3 pr-6 text-cyan-400/60">ACTIVE</td><td className="py-3 text-cyan-400/40">AGENT_ACTIVE</td></tr>
